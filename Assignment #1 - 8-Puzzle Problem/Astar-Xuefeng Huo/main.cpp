...
void AnimateSolution(string const initialState, string const goalState, string path)
{
    ...
    {
        cout << endl
             << "Nothing to animate." << endl;
    }
    else
    {
        cout << endl
             << "Animating solution..." << endl;
    ...
    for (int i = 0; i < path.length(); i++)
    {
        ...
        case 'U':
            ...
            break;
        case 'D':
            nextState = p->moveDown();
            cout << "[DOWN]" << endl;
            break;
        case 'L':
            nextState = p->moveLeft();
            cout << "[LEFT]" << endl;
            break;
        case 'R':
            nextState = p->moveRight();
            cout << "[RIGHT]" << endl;
            break;
        }
...
int main(int argc, char *argv[])
{
    ...
    else if ((typeOfRun == "single_run") || (typeOfRun == "animate_run"))
    {
        ...
        cout << setprecision(6) << setw(25) << std::setfill(' ') << std::right << endl
             << "Path Length:" << std::fixed << ' ' << setw(12) << pathLength << endl;
